...
class CategoryController(Controller):
    ...
    @post(path="/", summary="CreateCategory")
    async def create(self, category: Category, categories_repo: CategoryRepository) -> Category:
        """Create a new category."""
        return categories_repo.add(category)

    @patch(path="/{category_id:int}", summary="UpdateCategory")
    async def update(
